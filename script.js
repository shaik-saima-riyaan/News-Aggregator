...
function fetchNews(isSearching){
    ...
    fetch(url).then(response =>response.json()).then(data=>{
        ...
        const articlesWithImage = data.article.filter(article => article.urlToImage);

        if(articlesWithImage.length ===0 || articlesWithImage.length === lastArticleCount){
            displayNoMoreNews(); //want to create
            return;
        }
        ...
        console.log("There was an error fetchin the news", error);
        isLoading=false;
    });
}
